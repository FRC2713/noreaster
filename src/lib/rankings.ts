...
export function computeRankings(
  ...
  const idToStats = new Map<
    string,
    {
      ...
      countedAutoScoreMatches: number;
    }
  >();
  ...
  for (const m of matches) {
    ...
    red.totalScore += m.red_score!;
    red.countedScoreMatches += 1;
    blue.totalScore += m.blue_score!;
    blue.countedScoreMatches += 1;

    // Track auto scores - count all played matches, use 0 if auto score is null
    red.totalAutoScore += m.red_auto_score ?? 0;
    red.countedAutoScoreMatches += 1;
    blue.totalAutoScore += m.blue_auto_score ?? 0;
    blue.countedAutoScoreMatches += 1;
  }

  const statsArray = Array.from(idToStats.values()).map(s => {
    const avgRp = s.played > 0 ? s.totalRp / s.played : 0;
    const avgScore =
      s.countedScoreMatches > 0 ? s.totalScore / s.countedScoreMatches : 0;
    const avgAutoScore =
      s.countedAutoScoreMatches > 0
        ? s.totalAutoScore / s.countedAutoScoreMatches
        : 0;
    return {
      ...s,
      avgRp,
      avgScore,
      avgAutoScore,
    } as typeof s & { avgRp: number; avgScore: number; avgAutoScore: number };
  });
  ...
  function headToHeadWins(aId: string, bId: string) {
    ...
    for (const m of matches) {
      const involves =
        (m.red_alliance_id === aId || m.blue_alliance_id === aId) &&
        (m.red_alliance_id === bId || m.blue_alliance_id === bId);
      if (!involves) continue;
      if (m.red_score == null || m.blue_score == null) continue;
      if (m.red_score > m.blue_score) {
        if (m.red_alliance_id === aId) aWins++;
        else if (m.red_alliance_id === bId) bWins++;
      } else if (m.blue_score > m.red_score) {
        if (m.blue_alliance_id === aId) aWins++;
        else if (m.blue_alliance_id === bId) bWins++;
      }
    }
    return { aWins, bWins };
  }
  ...
  statsArray.sort((A, B) => {
    // 1. Ranking Point Average
    if (B.avgRp !== A.avgRp) return B.avgRp - A.avgRp;

    // 2. MATCHUP Results (head-to-head)
    ...
    // 4. Average AUTO Score
    if (B.avgAutoScore !== A.avgAutoScore)
      return B.avgAutoScore - A.avgAutoScore;

    // 5. Coin Flip (alphabetical by name)
    return A.name.localeCompare(B.name);
  });

  return statsArray.map(
    (s, idx) =>
      ({
        id: s.id,
        name: s.name,
