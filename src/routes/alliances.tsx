...
export default function AlliancesRoute() {
  const { user } = useAuth();
  const { alliances, teams, isLoading: alliancesLoading, error: alliancesError } = useAlliancesPolling();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            {alliances.length} alliance{alliances.length !== 1 ? 's' : ''} • {teams.filter(t => 
              alliances.some(a => a.teams.some(at => at?.id === t.id))
            ).length} teams assigned
          </p>
        </div>
        {user && (
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{rankings[0]?.name || 'N/A'}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {allMatches.filter(m => m.red_score !== null && m.blue_score !== null).length}
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        {alliances.length === 0 ? (
          <div className="col-span-full text-center py-12">
            <div className="text-muted-foreground mb-4">
              ...
              <p className="text-sm">Create your first alliance to get started</p>
            </div>
            {user && (
              <Button asChild>
            ...
            return (
              <Card key={alliance.id} className="group hover:shadow-lg transition-all duration-200">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <CardTitle className="text-xl group-hover:text-primary transition-colors">
                          <Link to={`/alliances/${alliance.id}`} className="hover:underline">
                            {alliance.name}
                          </Link>
                        </CardTitle>
                        {ranking && (
                ...
                <CardContent className="space-y-4">
                  {/* Team Slots */}
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground mb-3">Team Roster</h4>
                  ...
                  {ranking && (
                    <div className="grid grid-cols-2 gap-4 pt-2 border-t">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-muted-foreground">{ranking.losses}</div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {rankings.map((ranking, index) => (
                      <tr key={ranking.id} className="border-b last:border-b-0 hover:bg-muted/50">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                        ...
                        <td className="p-4 text-center font-medium text-green-600">{ranking.wins}</td>
                        <td className="p-4 text-center font-medium text-red-600">{ranking.losses}</td>
                        <td className="p-4 text-center font-medium text-blue-600">{ranking.ties}</td>
                        <td className="p-4 text-center font-medium">{ranking.avgRp.toFixed(1)}</td>
                        <td className="p-4 text-center font-medium">{ranking.avgScore.toFixed(0)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
